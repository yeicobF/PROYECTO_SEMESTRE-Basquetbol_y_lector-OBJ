...
// Método para obtener lista con todos los vértices del objeto.
vector <Vertex> Object::getFacesVertices(){
    vector <Vertex> auxVertexList;
    /* Se obtienen los aristas de cada cara,
        luego se busca el primer vértice de cada arista
        y se agrega a la lista de vértices.*/
    for(unsigned int i = 0; i < face_list.size(); i++)
        for(unsigned int j = 0; j < face_list[i].getFaceEdges().size(); j++)
            // Se obtienen los aristas de la cara actual.
            // Luego se obtiene su vértice inicial.
            auxVertexList.push_back(face_list[i].getFaceEdges()[j].getStartVertex());
